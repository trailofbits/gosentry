...
set +e
CGO_ENABLED=1 timeout 10m "${ROOT_DIR}/bin/go" test -fuzz=FuzzFocus --use-libafl --focus-on-new-code=true --catch-races=false --catch-leaks=false 2>&1 | tee "${output_file}"
status="${PIPESTATUS[0]}"
set -e
