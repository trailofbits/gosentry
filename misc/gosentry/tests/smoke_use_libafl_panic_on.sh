...
set +e
CGO_ENABLED=1 timeout 30s "${ROOT_DIR}/bin/go" test -fuzz=FuzzMyCustomError --use-libafl --focus-on-new-code=false --catch-races=false --panic-on=run 2>&1 | tee "${tmp_dir}/output-invalid-panic-on.txt"
status="${PIPESTATUS[0]}"
set -e
if [[ "${status}" -eq 0 ]]; then
...
set +e
CGO_ENABLED=1 timeout 5m "${ROOT_DIR}/bin/go" test -fuzz=FuzzMyCustomError --use-libafl --focus-on-new-code=false --catch-races=false 2>&1 | tee "${output_no_panic_on}"
status="${PIPESTATUS[0]}"
set -e
