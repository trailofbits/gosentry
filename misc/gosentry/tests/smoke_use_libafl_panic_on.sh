...
target_sym="gosentry.example/panic_on/myLog.myCustomError"
target_sym2="gosentry.example/panic_on/myLog.myCustomWarning"
target_sym_wildcard="gosentry.example/panic_on/myLog.myCustom*"
target_sym_nodot="test_go_panicon.(*Logger).Error"

# Invalid --panic-on should error out early.
cd "${ROOT_DIR}/test/gosentry/examples/panic_on/myLog"
set +e
CGO_ENABLED=1 timeout 30s "${ROOT_DIR}/bin/go" test -fuzz=FuzzMyCustomError --use-libafl --focus-on-new-code=false --catch-races=false --catch-leaks=false --panic-on=run 2>&1 | tee "${tmp_dir}/output-invalid-panic-on.txt"
status="${PIPESTATUS[0]}"
set -e
if [[ "${status}" -eq 0 ]]; then
...
set +e
CGO_ENABLED=1 timeout 5m "${ROOT_DIR}/bin/go" test -fuzz=FuzzMyCustomError --use-libafl --focus-on-new-code=false --catch-races=false --catch-leaks=false 2>&1 | tee "${output_no_panic_on}"
status="${PIPESTATUS[0]}"
set -e
...
if grep -Eq "${GOSENTRY_LIBAFL_CRASH_RE}" "${output_no_panic_on}"; then
